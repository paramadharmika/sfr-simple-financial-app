...
public class SavePaymentChannelCommand implements ISavePaymentChannelCommand {
    ...
    @Inject
    public SavePaymentChannelCommand(OkHttpClient client, Gson gson, IAppConfiguration appConfiguration) {
        this.client = client;
        this.gson = gson;
    ...
    @Override
    public void executeAsync(PreferredPaymentModel param) {
        RequestBody body = RequestBody.create(JSON, gson.toJson(param));

        Request request = new Request.Builder()
                .url(appConfiguration.getBaseUrl() + "savePayment")
                .post(body)
                .build();
